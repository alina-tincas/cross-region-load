...
```
cd src/AlinaCrossRegionLoad.Web
dotnet build
dotnet run
```
...
Browse to the URLs from the terminal output of `dotnet run` to see your Umbraco site or alternatively open the `src/AlinaCrossRegionLoad.Web/AlinaCrossRegionLoad.Web.csproj` file in Visual Studio or JetBrains Rider.
...
Now you can type `dotnet run` from within the `src/AlinaCrossRegionLoad.Web` folder to start the project. When running it for the first time, the database schema will be automatically created (with `"InstallUnattended": true` in appsettings.Development.json), so it starts up ready for use.
...
.
├── src
│   └── AlinaCrossRegionLoad.Web                              (Project folder - can be renamed)
│       ├── Properties                              
...
│       └── AlinaCrossRegionLoad.Web.csproj                   (The Project file used to build and run the Umbraco project - can be renamed)
├── .dockerignore                             
├── .editorconfig                           
├── .gitattributes                            
├── .gitignore                            
...
```
[project]
base = "src/AlinaCrossRegionLoad.Web"
csproj = "AlinaCrossRegionLoad.Web.csproj"
```
...
So you could rename `AlinaCrossRegionLoad.Web.csproj` to `MyAwesomeProject.Web.csproj` and have one or more additional class library projects such as `MyAwesomeProject.Code.csproj`
...
> Recommendation: When creating new projects along side the default AlinaCrossRegionLoad.Web, we recommend that they are added to the src folder in the git repository.
